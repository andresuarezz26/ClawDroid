...
class AndroidAgentFactory @Inject constructor(
  ...
  fun createAgent(
    ...
    Log.i(
      TAG,
      "createAgent called with provider=${config.provider}, model=${config.model}, historySize=${conversationHistory.size}"
    )
    ...
    Log.i(
      TAG,
      "Resolved model: $model, maxIterations=${config.maxIterations}, temperature=${config.temperature}"
    )

    val agentType = createAgentByType(config.agentType)

    val systemPromptWithHistory = buildSystemPrompt(conversationHistory, agentType.systemPrompt())
    ...
    return AIAgent(
      ...
      toolRegistry = agentType.toolRegistry()
  ...
  private fun buildSystemPrompt(history: List<ChatMessage>, prompt: String): String {
    if (history.isEmpty()) {
      return prompt
    }
